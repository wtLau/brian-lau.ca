...
Frist, we need to acknowledge that "code is a liability", what it provide might be an asset, but down to its core, code is simply a mainenance task for someone in the future. Much like a real estate on a piece of land, it is a shelter, though its obvious for one to be stay comfortably. Or like the fuel that an rocket carries, it has tremedous weight, but it is necessary for it to reach orbital altitude.
...
Code review is a critical piece of process that acts as a form of open communication with peers. In my experience, I've been in both through both environment at various workplace, I would now much prefer codebase where code review is an essential part of the development process. I've learned a lot about my team's coding styles and approaches when trying to solve a problem, sometime, I can even be helpful to point out some interesting bugs. It implicitly synchonize the team's code quality, including small changes, fast feedback loop, hence maintain developer's satisfaction and production velocity.

That's it! Hopefully, this helps explain my approach to implementing authentication for my [website](https://www.brian-lau.ca).

## Action items

<!-- Things that be worked on -->
